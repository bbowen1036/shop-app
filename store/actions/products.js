...
export const deleteProduct = (productId) => {
  return async (dispatch, getState) => {
    ...
    const response = await fetch(
      `https://shop-app-bb69d-default-rtdb.firebaseio.com/products/${productId}.json?auth=${token}`,
      {
        method: "DELETE",
      }
    );
...
export const createProduct = (title, description, imageUrl, price) => {
  return async (dispatch, getState) => {
    ...
    const token = getState().auth.token
    const response = await fetch(
      `https://shop-app-bb69d-default-rtdb.firebaseio.com/products.json?auth=${token}`,
      {
        method: "POST",
        headers: {
...
export const updateProduct = (id, title, description, imageUrl) => {
  return async (dispatch, getState) => {
    const token = getState().auth.token
    const response = await fetch(
      `https://shop-app-bb69d-default-rtdb.firebaseio.com/products/${id}.json?auth=${token}`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
